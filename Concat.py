...
# Bonus: Make a prediction for a new movie
def predict_rating(year, genre_sentiment, is_weekend, is_holiday_release, is_peak_season, season_features, director_avg_rating, genre_avg_rating):
    features = [year, genre_sentiment, is_weekend, is_holiday_release, is_peak_season] + season_features + [director_avg_rating, genre_avg_rating]
    return model.predict(np.array([features]))[0]

# Example usage with new features
season_features = [0] * len([col for col in df.columns if col.startswith('Season_')])  # Adjust as needed
predicted_rating = predict_rating(2024, 0.5, 1, 0, 1, season_features, 7.0, 6.5)
print(f'Predicted Rating for a movie in 2024 with genre sentiment 0.5, director average rating 7.0, and genre average rating 6.5: {predicted_rating:.2f}')
